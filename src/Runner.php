...
class Runner
{
    ...
    public function run(): static
    {
        if (null !== $this->getIo()) {
            $this
                ->getIo()
                ->writeln(messages: 'scanning domains.')
            ;
            $this
                ->getIo()
                ->progressStart(max: count(value: $this->results))
            ;
        }
        ...
        if (null !== $this->getIo()) {
            ...
            $this
                ->getIo()
                ->writeln(messages: 'finished scanning domains.')
            ;
            $this
                ->getIo()
                ->writeln(messages: 'ready to generate the results.')
            ;
        }
    ...
    protected function checkDomain(Result $result): static
    {
        ...
        ob_get_clean();
        if (false === $result->isValid() || false === $certificate instanceof \Spatie\SslCertificate\SslCertificate) {
            return $this;
        }
        $result
            ...
            ->setValidUntil(validUntil: $certificate->expirationDate())
            ->setValidUntilDays(validUntilDays: (int)$certificate
                                                  ->expirationDate()
                                                  ->diffInDays() * -1)
        ;
